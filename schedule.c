...
int check_if_possible_replacements(employee_s possible_replacements[],
  ...
  do {
    ...
    do {
      ...
      while (schedule[shift].day == day && schedule[shift].month == month) {
        sprintf(display_choice[k++],
                "EMPLOYEE: %-20s TIME: %5.2f-%5.2f. ROLE: %s",
    ...
    if (i == 0) {
      sprintf(display_choice_error_messages[0],
              "ERROR. NO SHIFTS FOUND ON SPECIFIED DATE: %d/%d", day, month);
    ...
    while (possible_replacements[j].name[0] != '\0') {
      if (possible_replacements[j].points > 0) {
        sprintf(display_choice[k++], "REPLACEMENT SCORE: %-15d %-30s%s",
