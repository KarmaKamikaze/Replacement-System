...
void scan_name_add_employee(employee_s employees[], int current_employee) {
  char throwaway_string[10]; /*throwaway_string is used to prevent
                                scanf-overflow*/
  printf("FULL NAME: ");
...
void scan_number_of_positions(employee_s employees[], int current_employee) {
  ...
  do {
    ...
    gets(throwaway_string);
    if (employees[current_employee].number_of_positions > MAX_POSITIONS ||
...
void print_employee_after_adding(employee_s employees[], int current_employee,
                                 char positions_str_arr[MAX_POSITIONS][20]) {
  int i;
